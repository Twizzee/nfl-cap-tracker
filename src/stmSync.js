...
export function parseStmRoster(html){
  ...
  while((tr=trRe.exec(html))){
    ...
    rows.push({name,position:pos,capHit2026:money(cells[2]),worth2026:money(cells[3]),baseSalary2026:money(cells[4]),deadCapCut2026:money(cells[5]),cutSavings2026:money(cells[6])});
  }
  ...
  let m;
  while((m=re.exec(body))){const name=cleanName(m[1]);if(/^(?:Player|Cut Savings|Dead Money|Cap Scenario)/i.test(name))continue;const k=normName(name);if(seen.has(k))continue;seen.add(k);rows.push({name,position:m[2],capHit2026:money(m[3]),worth2026:money(m[4]),baseSalary2026:money(m[5]),deadCapCut2026:money(m[6]),cutSavings2026:money(m[7])});}
  return rows;
}
...
function mergeTeam(state,abbr,data,now){
  let matched=0,added=0; const teamPlayers=state.players.filter(p=>p.team===abbr&&p.status!=='removed');
  for(const c of data.rows){let p=teamPlayers.find(x=>normName(x.name)===normName(c.name));if(!p){p={team:abbr,name:c.name,position:c.position,status:'active',source:'StickToTheModel contract sync'};state.players.push(p);teamPlayers.push(p);added++;}Object.assign(p,c);p.position=p.position||c.position;p.contractStatus='verified';p.contractSource='StickToTheModel';p.contractSourceUrl=data.url;p.contractUpdatedAt=now;p.sourceChecks={...(p.sourceChecks||{}),STM:true};matched++;}
  const t=state.teams.find(x=>x.abbr===abbr);if(t){t.contractsUpdatedAt=now;t.contractPlayerCount=data.rows.length;t.contractSource='StickToTheModel';if(data.summary){Object.assign(t,data.summary);t.capSource='StickToTheModel';t.capUpdatedAt=now;}}
  return {abbr,matched,added,players:data.rows.length,url:data.url};
}
