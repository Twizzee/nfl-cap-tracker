...
import { syncOtcContracts } from './otcSync.js';
...
function summarize(s) {
  return {
    ...
    contractTerms: s.players.filter(p => p.status !== 'removed' && hasContractTerms(p)).length,
    freeAgents: (s.freeAgents || []).length,
    news: (s.news || []).length,
    lastSync: s.lastSync,
    lastContractSync: s.lastContractSync,
    lastSpotracSync: s.lastSpotracSync,
    lastOtcSync: s.lastOtcSync,
...
function validateSync(s,result={}){
  ...
  const withTerms=active.filter(p=>hasContractTerms(p)&&(Number(p.apy||0)>0||Number(p.totalValue||0)>0)).length;
  const withGuarantee=active.filter(p=>Number(p.totalGuaranteed||p.guaranteed2026||0)>0).length;
  const contractSourceOk=Number(result.spotrac?.teams||0)>=25||Number(result.otc?.matched||0)>=300;
  const checks={
    ...
    contractTerms:contractSourceOk&&withTerms>=300,
    guarantees:withGuarantee>=150,
  ...
  return {ok:Object.values(checks).every(Boolean),checks,counts:{active:active.length,withCap,withTerms,withGuarantee,news:(s.news||[]).length,freeAgents:(s.freeAgents||[]).length}};
}
...
async function fullSync(s){
  const result={};
  result.roster=await syncEspn(s).catch(e=>({error:String(e.message||e),teamsSynced:0,fullRosters:0,validated:false,failures:[]}));await writeState(s);
  ...
  result.pfn=await syncAllContracts(s).catch(e=>[{error:String(e.message||e)}]);await writeState(s);
  s.lastFullSync=new Date().toISOString();result.validation=validateSync(s,result);s.lastValidation=result.validation;s.syncLog=s.syncLog||[];
  const newsSources=Object.entries(result.news.sources||{}).map(([k,v])=>`${k} ${v.ok?v.count:'ERR'}`).join(', ');
  s.syncLog.unshift({timestamp:s.lastFullSync,status:result.validation.ok?'ok':'partial',message:`Pipeline: ESPN rosters ${result.roster.teamsSynced||0}/32, STM ${result.stm.teams||0}/32, Spotrac ${result.spotrac.teams||0}/32, OTC ${result.otc.matched||0} matched, free agents ${result.freeAgents.players||0}, news ${result.news.count||0}${newsSources?` (${newsSources})`:''}, PFN caps ${result.teamCaps.teams||0}/32. Validation ${result.validation.ok?'PASS':'PARTIAL'}.`});s.syncLog=s.syncLog.slice(0,300);await writeState(s);return result;
}
