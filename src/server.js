...
async function readCanonicalState(){
 ...
 const merged=canonicalizePlayers(s);
 if(merged>0)await writeState(s);
 return s;
}
...
app.get('/api/health',async(_req,res)=>{try{const s=await readCanonicalState();res.json({ok:true,storage:storageMode(),summary:summarize(s),validation:s.lastValidation||null,now:new Date().toISOString()})}catch(error){res.status(500).json({ok:false,error:String(error.message||error)})}});
app.get('/api/debug/contracts',async(_req,res)=>{try{const s=await readCanonicalState();res.json({ok:true,primarySource:'https://www.spotrac.com/nfl/contracts',lastFullSync:s.lastFullSync,lastSpotracSync:s.lastSpotracSync,diagnostics:contractDiagnostics(s),lastValidation:s.lastValidation||null})}catch(error){res.status(500).json({ok:false,error:String(error.message||error)})}});
...
app.get('/api/state',async(_req,res)=>{try{res.json(await readCanonicalState())}catch(error){res.status(500).json({error:String(error.message||error)})}});
...
async function runLockedSync(){const lock=await acquireSyncLock('full-sync',25);if(!lock.acquired)return{busy:true};try{const s=await readCanonicalState(),result=await fullSync(s);return{busy:false,result,summary:summarize(s)}}finally{await releaseSyncLock('full-sync',lock.owner)}}
